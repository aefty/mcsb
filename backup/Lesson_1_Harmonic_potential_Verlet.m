...
while timestep<nsteps-3
    ...
    plot([-r(timestep)./2 r(timestep)./2],[0 0],'r-',-r(timestep)./2,0,'ko',r(timestep)./2,0,'b+','MarkerSize',10,'MarkerFaceColor','w','LineWidth',5.0);
    set(gca,'FontSize',16)
   ...
   % set(gca,'YTick',[ ])
    ...
    plot(time(1:end-1),POT,'b-',time(1:end-1),KIN,'g-',time(1:end-1),H,'r-',...
    time(timestep),POT(timestep),'bo',time(timestep),KIN(timestep),'go',time(timestep),H(timestep),'ro','MarkerSize',10,'MarkerFaceColor','w');
    ...
    subplot(1,3,2)
    set(gca,'FontSize',16)
    plot(r(3:end-1),v(3:end)./m,...
    r(timestep),v(timestep)./m,'ro','MarkerSize',10,'MarkerFaceColor','w');
    title('Phase Space');
    xlabel('Position')
    ylabel('Momentum')
